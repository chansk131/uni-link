...
export default class AddDurationScreen extends React.Component {
  state = {
    uid: null,
    key: null,
    section: null,
  ...
  submit = () => {
    if (
      this.state.form.duration.valid &&
      this.state.key != null &&
      this.state.uid != null
    ) {
      try {
  ...
  addToDatabase = () => {
    let updates = {}
    updates[
      '/products/' + this.state.key + '/duration'
    ] = this.state.form.duration.value
  ...
  render() {
    return (
      ...
      >
        <Label label={'Prefered duration'} required={true} />
        <TextInput
          autoFocus
